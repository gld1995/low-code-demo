...
export function DroppedItem(
  ...
  const { setSchemaList, activeSchema, forceRender } =
    useContext(ConfigsContext);
  const { id, type, configs } = schema;
  const StandardInput = itemStore[type].StandardInput;
  ...
  const deleteSchema = (e: React.MouseEvent) => {
    ...
    setSchemaList((pre: any) => pre.filter((it: any) => it.id !== id));
    forceRender();
  };
  ...
  return (
    ...
    >
      <Button type="text" className="delete-butt" onClick={deleteSchema}>
        X
      </Button>
